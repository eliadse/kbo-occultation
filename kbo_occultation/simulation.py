...
def apply_stellar_disk_2d(x_m, intensity_radial, r_grid_m, star_radius_m, impact_parameter_m, n_star_side):
    ...
    # --- Convolution: for each chord position, average I(r) over the
    #     disk sample points. Broadcast in chunks over x_m so the
    #     (chunk, n_disk) temporaries stay bounded in memory. ---
    convolved = np.zeros_like(x_m)
    chunk = 2048

    for start in range(0, len(x_m), chunk):
        xs = x_m[start:start + chunk, None]
        r = np.sqrt((xs + dx[None, :])**2 + (impact_parameter_m + dy[None, :])**2)
        vals = np.interp(r.ravel(), r_grid_m, intensity_radial, left=1.0, right=1.0)
        convolved[start:start + chunk] = vals.reshape(r.shape).mean(axis=1)
